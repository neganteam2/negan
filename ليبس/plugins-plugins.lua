...
if tonumber(msg.from.id) ~= tonumber(sudo_id) then return " هذا الاوامر للمطور الاساسي فقط 🔖┇" end
  ...
  for k, v in pairs( plugins_names( )) do  
      -- get the name 
      v = string.match (v, "(.*)%.lua") 
sendDocument(msg.to.id, "./plugins/"..v..".lua", msg.id, "🔖┇ الملف مقدم من قناة نيــغان 🔖┇ \n🔖┇ تابع قناة السورس @n_n_e_n_n69")
...
sendDocument(msg.to.id, "./plugins/"..file..".lua", msg.id, "🔖┇ الملف مقدم من قناه نيــــغان 🔖┇ \n🔖┇ تابع قناة السورس @n_n_e_n_n69")
end
end
end
