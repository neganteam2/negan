...
if not tostring(SUDO):match('%d+') then
  ...
  config = {
   ...
   "plugins",
    ...
    sudo_users = {786642704, SUDO},--Sudo users
    master_id = SUDO, 
    token_bot = token, 
    botname = (botname or "𝗡 𝗘 𝗚 𝗔 𝗡 ࿓"), 
    sudouser = (sudouser or "@l_na_lbot"), 
...
function load_config( )
  ...
  if not f then
    print ("\n🔖┇ جاري انشاء الكونفك :\n🔖┇ خلي ايديك والتوكن واسم البوت ومعرفك كمطور \n🔖┇ V21 وسوف يتم  تشغيل  السورس نيغـــان توكن الاصدار  ")
    create_config()
  else
    f:close()
  end
...
function bot_run()
	...
	bot = bot.result
	local runlog = "🔖┇ معرف بوتك : @"..bot.username.."\n	🔖┇ يعمل ع سورس نيغـــان توكن الاصدار V21\n🔖┇ تابع قناه السورس @n_n_e_n_n69"
	print(runlog.."\n┇-----------------------------------")
